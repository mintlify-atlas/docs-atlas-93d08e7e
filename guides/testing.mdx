...
<CardGroup cols={2}>
  <Card title="Emulator (Integration)" icon="server" href="/guides/testing">
  ...
  <Card title="MagicMock (Unit)" icon="flask" href="/guides/testing">
...
For continuous integration pipelines, spin up the emulator as a service container. The repository ships with a `docker-compose.test.yml` that starts the emulator and runs the test suite:

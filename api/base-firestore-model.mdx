...
Persist a new document to Firestore. If `id` is `None`, Firestore generates a unique document ID and assigns it to the instance. Raises `RuntimeError` if an explicit `id` is provided but a document with that ID already exists.
...
Update fields on an **existing** Firestore document. The instance must have a non-`None` `id`. Only fields that pass the exclusion rules are sent to Firestore.
...
Delete the document from Firestore. The instance must have a non-`None` `id`.
